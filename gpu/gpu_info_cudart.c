...
void cudart_check_vram(cudart_handle_t h, int i, mem_info_t *resp) {
  ...
  if (ret != CUDART_SUCCESS) {
    ...
    // TODO any other useful properties to extract from props?
  }
  ...
  LOG(h.verbose, "[%s] CUDA totalMem %lu\n", resp->gpu_id, memInfo.total);
  LOG(h.verbose, "[%s] CUDA freeMem %lu\n", resp->gpu_id, memInfo.free);

  resp->total = memInfo.total;
  resp->free = memInfo.free;

  // Best effort
  ret = (*h.cudaDeviceGetAttribute)(&resp->major, cudartDevAttrComputeCapabilityMajor, i);
  if (ret != CUDART_SUCCESS) {
    LOG(h.verbose, "[%s] device compute capability lookup failure: %d\n", resp->gpu_id, ret);
    resp->major = 0;
    resp->minor = 0;
  } else {
    ret = (*h.cudaDeviceGetAttribute)(&resp->minor, cudartDevAttrComputeCapabilityMinor, i);
    if (ret != CUDART_SUCCESS) {
      LOG(h.verbose, "[%s] device compute capability lookup failure: %d\n", resp->gpu_id, ret);
      resp->major = 0;
      resp->minor = 0;
